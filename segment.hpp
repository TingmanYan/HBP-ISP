...
protected:
    void
    img_CPU_to_GPU(const cv::Mat &, float4 *&, const double);
    void
    img_GPU_to_CPU(const float *, cv::Mat &);

    void
    compute_1nn_grid(const float4 *, int *, int2 *, float *, float *);
    void
    init_data_mean(const float4 *, float *);
    void
    initialize_image_label(int *, const int);
    int
    get_target_clus(const float *, int *, const int, const int);
    int
    reduce_boundary_dist(int2 *&, int2 *&, float *&, float *&, const int *,
                         const int);
    bool
    save_output(std::vector<cv::Mat> *, const int, const int, const int);
    void
    compute_dist_inner_max(const int *, const float *, float *, const int,
                           const int);
    void
    compute_1nn(const int2 *, const float *, int *, const int, const int);
    int
    compute_dist_reduce(const int2 *, const float *, int2 *, float *, const int);
    void
    reduce_data_mean(float *, float *, const int *, const int, const int,
                     const int);
    void
    compute_dist_mean(float *, const int2 *, float *, const int, const int);
    void
    pernalize_dist(const int2 *, const float *, float *, const int);

private:
    void
    update_image_label(int *, int *, int *, int *);
    void
    push_label_isp(const int, const int);
    cv::Mat
    draw_segmentation(int *, int *);

    void
    compute_dist_pos(const float *const, const int *const, float *const,
                     int *const, const int, const int);
    void
    compute_scaned_pos(float *const, int *const, int *const, const int);
    std::pair<int, int>
    set_pre_clus_label(int *const, const int *const, const int, const int);

public:
    void
    run_ms(std::vector<cv::Mat> *segs = nullptr);
...
public:
    ...
    int
    g_mean_channels() const
    {
        return this->mean_channels;
    }
    std::vector<int>
    g_num_clus_isp() const
    {
        return this->num_clus_isp;
    }
    std::vector<int>
    g_num_bd_isp() const
    {
        return this->num_bd_isp;
    }
    int *
    g_isp_clus_d() const
    {
        return this->isp_clus_d;
    }
    float *
    g_isp_mean_d() const
    {
        return this->isp_mean_d;
    }
    int2 *
    g_isp_bd_d() const
    {
        return this->isp_bd_d;
    }
    int *
    g_isp_bdl_d() const
    {
        return this->isp_bdl_d;
    }
...
public:
     ...
     */
    SegHAC(const cv::Mat &src, const Linkage link, const int num_nb, const double sigma, const SegFormat seg_format, const int target_clus, const bool show_cycle) : width(src.cols), height(src.rows), im_size(width * height), img_grids((width + m_blocks.x - 1) / m_blocks.x, (height + m_blocks.y - 1) / m_blocks.y), img_grid((im_size + m_block - 1) / m_block), num_bd(im_size * num_nb), m_link(link), use_mean(link == CentoridLink || link == WardLink), num_nb(num_nb), m_sigma(sigma), m_seg_format(seg_format), m_target_clus(target_clus), show_cycle(show_cycle)
    {
        ...
        else if (m_seg_format == LabelISPFormat)
        {
            cudaMalloc(&isp_clus_d, sizeof(int) * im_size * 2);
            cudaMalloc(&isp_mean_d,
                       sizeof(float) * im_size * mean_channels * 2);
            cudaMalloc(&isp_bd_d, sizeof(int2) * num_bd * 2);
            cudaMalloc(&isp_bdl_d, sizeof(int) * num_bd * 2);

            cudaMalloc(&bdl_d, sizeof(int) * num_bd);
            cudaMalloc(&bdl_rd_d, sizeof(int) * num_bd);
        }
