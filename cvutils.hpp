#ifndef _CVUTILS_HPP_
#define _CVUTILS_HPP_
...
namespace cvutils
{
	namespace io
	{
		...
		static bool writeMatBinary(std::ofstream &ofs, const cv::Mat &out_mat)
		{
			...
			{
				int s = 0;
				ofs.write((const char *)(&s), sizeof(__int32_t));
				return true;
			}
			__int32_t rows = out.rows;
			__int32_t cols = out.cols;
			__int32_t type = out.type();

			ofs.write((const char *)(&rows), sizeof(__int32_t));
			ofs.write((const char *)(&cols), sizeof(__int32_t));
			ofs.write((const char *)(&type), sizeof(__int32_t));
		...
		static bool readMatBinary(std::ifstream &ifs, cv::Mat &in_mat, bool readHeader = true)
		{
			...
			if (readHeader)
			{
				__int32_t rows, cols, type;
				ifs.read((char *)(&rows), sizeof(__int32_t));
				if (rows == 0)
				{
					return true;
				}
				ifs.read((char *)(&cols), sizeof(__int32_t));
				ifs.read((char *)(&type), sizeof(__int32_t));
