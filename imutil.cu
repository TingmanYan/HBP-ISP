...
// __global__ void
// img_down_samp(const float4* const img_f4_d,
//               float* const img_samp_d,
//               const int samp,
//               const int samp_width,
//               const int samp_height,
//               const int samp_size,
//               const int width)
// {
//   int x = blockIdx.x * blockDim.x + threadIdx.x;
//   int y = blockIdx.y * blockDim.y + threadIdx.y;
//   if (x >= samp_width || y >= samp_height)
//     return;

//   int index_samp = y * samp_width + x;
//   int index = y * samp * width + x * samp;
//   float4 val = img_f4_d[index];
//   img_samp_d[index_samp] = val.x;
//   img_samp_d[index_samp + samp_size] = val.y;
//   img_samp_d[index_samp + samp_size * 2] = val.z;
// }
...
void compute_gauss_kernel(float *const filter_d, const double sigma, const int S)
{
...
void img_gauss_blur(float4 *&img_d,
                    float4 *&buf_d,
...
void img_sobel_grad(float4 *&img_d,
                    float4 *&buf_d,
                    const int width,
                    const int height)
{
